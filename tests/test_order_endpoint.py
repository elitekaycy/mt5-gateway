...
OrderResult = namedtuple(
    "OrderResult", "retcode comment volume price order deal sl tp"
)
...
def client(monkeypatch):
    ...
    monkeypatch.setattr(order_route.mt5, "symbol_select", lambda *_: True, raising=False)
    monkeypatch.setattr(
        order_route.mt5,
        "symbol_info",
    ...
    monkeypatch.setattr(
        order_route.mt5, "last_error", lambda: (0, "ok"), raising=False
    )
...
def test_same_key_with_different_http_request_returns_conflict(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda request: (
            calls.append(request)
            or OrderResult(10009, "done", 0.1, 1.10, 10, 11, 0, 0)
        ),
        raising=False,
    )
    headers = {"Idempotency-Key": "strategy-order-1"}
